...
describe('fulltext-engine', function() {
  var db, dbPath = path.join(__dirname, '..', 'data', 'test-db'),
      manifestPath = path.join(__dirname, '..', 'data', 'manifest.json');

  beforeEach(function(done) {
    rimraf.sync(dbPath);
    rimraf.sync(manifestPath);
    done();
  });

  function createServer(db, manifestPath, cb) {
    db.methods['query'] = { type: 'readable' };
    multilevel.writeManifest(db, manifestPath);

    var _server = net.createServer(function (con) {
      var server = multilevel.server(db);
      con.pipe(server).pipe(con).on('error', console.error);
    }).listen(0, function () {
      cb(null, _server);
    });
  }
  ...
  it('should be able to index a document (non fuzzy)', function(done) {
    ...
    function populateDb() {
      ...
      db.batch(testData(), function (err) {
        ...
        createServer(db, manifestPath, createClient);
      });
    }

    function createClient(err, server) {
      if (err) return done(err);
      var db = multilevel.client(require(manifestPath));
      var con = net.connect(server.address().port);
